...
if (!empty($_POST)){
    ...
    if ($is_ok){
        // save
        $r = save_post($post_title, $post_desc, $post_thumb, $category_id, $post_content);
        unset($post_title);
        unset($post_desc);
        unset($post_thumb);
        unset($category_id);
    ...
    <body>
        <form style="width: 50%; margin: auto;" action="" method="post" class="vertical">
            <fieldset>
                ...
                <div class="notice success <?php if($r) {echo 'show';} else {echo 'hide'; }?>">Thêm mới bài viết thành công !!!</div>
